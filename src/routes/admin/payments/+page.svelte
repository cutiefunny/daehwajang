        ...
        try {
            ...
            payments = querySnapshot.docs.map(doc => {
                ...
                return {
                    ...
                    productName: data.productName || data.orderName || '-', // orderName도 허용
                ...
                {#if paginatedPayments.length > 0}
                    {#each paginatedPayments as payment}
                        <tr>
                            ...
                            <td>
                                <span class="method-badge {payment.method === '카드' ? 'card' : 'other'}">
                                    {payment.method}
                                </span>
                            </td>
                            <td>
...
<style>
    ...
    .method-badge.card { background-color: #e9d8fd; color: #6b46c1; } 
    .method-badge.other { background-color: #bee3f8; color: #2b6cb0; } 

    .status-badge {
        padding: 4px 8px; border-radius: 4px; font-size: 11px; font-weight: 600;
    }
    .status-badge.paid { color: #276749; background-color: #f0fff4; }
