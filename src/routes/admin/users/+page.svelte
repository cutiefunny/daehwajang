<script>
	...
	async function toggleStatus(user) {
		...
		const confirmMsg = newStatus === 'suspended' 
			? `${user.nickname} 님의 활동을 정지하시겠습니까?` 
			: `${user.nickname} 님의 활동 정지를 해제하시겠습니까?`;
	...
	{:else}
		<table>
			...
			<tbody>
				{#if paginatedUsers.length > 0}
					{#each paginatedUsers as user}
						<tr>
							...
							<td>{user.age}세</td>
							<td>
								<span class="badge membership {user.membership.toLowerCase()}">
									{user.membership}
								</span>
							</td>
							...
							<td>
								<button class="action-btn" on:click={() => toggleStatus(user)}>
				...
				{:else}
					<tr>
						<td colspan="8" class="empty-message">검색 결과가 없습니다.</td>
					</tr>
				{/if}
			</tbody>
		</table>
...
<style>
	...
	.action-btn { padding: 6px 12px; border: 1px solid #e2e8f0; background: white; border-radius: 6px; font-size: 12px; cursor: pointer; transition: all 0.2s; }
